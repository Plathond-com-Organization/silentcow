      ...
      <section className="container py-10">
        <div className="flex flex-wrap -mx-2">
          {whats.map((what, index) => (
            <WhatCardItem cardNum={index + 1} cardText={what.title} key={index} />
          ))}
        </div>
      </section>
      <section className="container">
        {whats.map((what, index) => (
          <WhatSectionItem what={what} key={index}></WhatSectionItem>
        ))}
      </section>
    </Layout>
  )
...
export const WhatSectionItem = ({ what, index }) => {
  return (
    <div
      id={`what${index}`}
      className="flex flex-col lg:flex-row bg-custom-gray text-white my-10 justify-between rounded-lg hover:shadow-xl overflow-hidden tr-fast"
    >
